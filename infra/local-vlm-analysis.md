...
**UPDATE 2026-06-16 — M5 Max (48 GB): always-on model swapped 9B → Qwen3-VL-30B-A3B; 27B retired.** After migrating to the Mac Studio M5 Max (48 GB), a 3-way deep-pass bake-off (`bakeoff_models.py`, 35 has_person photos, identical prompts) settled the model question on Karl's own images: **Qwen3-VL-30B-A3B won** (mean 6.16, **3.64 s/img — faster than the old 9B's ~4.3 s**, MoE ~3B active) vs **dense Qwen3-VL-32B** (mean 5.55, 11.4 s/img — slower *and* lower, retired on merit) vs the 9B baseline (5.60). Cross-model Spearman agreement is low (~0.54–0.65) → aesthetic scoring is inherently noisy; a re-score visibly re-ranks the gallery. **Hard RAM lesson:** even 48 GB only frees ~17–22 GB in normal use (macOS wired + stacked Claude sessions), so you can't hold the 9B *and* a 19 GB model — the upgrade had to **replace** the 9B, not run beside it. Also, at night/idle the watchdog auto-starts the 27B (~16 GB) and silently steals the candidates' RAM — `touch ~/.openclaw/watchdog/disable-27b` to stop it (left set permanently; the 27B is dead weight now that openclaw runs on `claude -p`). **Wiring:** `mac-watchdog.sh` `CHAT_MODEL`, `triage-runner.sh` `MLX_VLM_MODEL`, and openclaw.json's `mlx-vlm-fast` provider (:8081) + model fallback all → `mlx-community/Qwen3-VL-30B-A3B-Instruct-4bit` (:8081 now serves it, ~16 GB resident). Triggered a **graceful re-score** of all 31,199 has_person items (cleared `deep_done_at` only; kept `aggregated_at`/`weighted_score` so the published gallery shows old 9B scores until each is re-done over ~1–2 days). The `aesthetic-deep`/`:8080` "27B later" path is now obsolete.
